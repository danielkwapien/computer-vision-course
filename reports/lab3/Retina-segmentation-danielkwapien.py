...
def th_based_segmentation_bottomhat(im, indx):
    # Getting retina mask
    retina_mask = getting_retina_mask(im)
    ...
    # Apply Top-Hat
    im = closing(im, disk(8)) - im

    # Apply Otsu Thresholding
    global_thresh = threshold_otsu(im)
    predicted_mask = im > 0.85 * global_thresh
    predicted_mask = predicted_mask & retina_mask  # Keep only the segmentation within the mask

    return predicted_mask, retina_mask


def clustering_segmentation(im, indx, k=2):
    # Getting retina mask
    retina_mask = getting_retina_mask(im)

    # Apply binary erosion to clean up the retina mask
    structure = np.ones((10, 10), dtype=int)
    retina_mask = binary_erosion(retina_mask, structure=structure)
    ...
    # Reshape the image for K-Means (each pixel is a data point)
    image_reshaped = im[retina_mask].reshape(-1, 1)

    # Apply K-Means clustering
    kmeans = KMeans(n_clusters=k, random_state=0)
    kmeans.fit(image_reshaped)

    # Get the clustered labels and reshape them back to the original image shape
    clustered_labels = np.zeros_like(im)
    clustered_labels[retina_mask] = kmeans.labels_

    # Choose the cluster with the lowest intensity values as the segmentation
    predicted_mask = clustered_labels == clustered_labels.min()
...
def segmentation_evaluation(train_im, train_mk):
    ...
    for indx, img in enumerate(train_im):
        ...
        pred_mask, retina_mask = th_based_segmentation_bottomhat(img, indx)
